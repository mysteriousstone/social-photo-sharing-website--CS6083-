...
<?php
$fname=$_GET['fname'];
$sql=mysql_query("SELECT distinct cname FROM friend_circle
WHERE cname not in (select cname from friend_circle where
friendname='$fname' and
username='$uname')");
while($row=mysql_fetch_array($sql))
{
echo "<a href=\"cadd1.php?fname=".$fname."&&cname=".$row[0]."\">".$row[0]."</a>"."  ";
}
?>